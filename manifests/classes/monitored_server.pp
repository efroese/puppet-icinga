...
# define this host for nagios
    icinga::host {
        $fqdn :
            hostgroups =>
            "${domain},${operatingsystem},${virtual},${architecture}",
    }
}
class icinga::monitored::server::nrpe ($ensure = "present") {
    class {
        "icinga::monitored::server" :
    }
    ...
    if $kernel == "Darwin" {
        ...
        exec {
            "restart-nagios-nrpe" :
                command =>
                "launchctl stop org.macports.nrpe && sleep 2 && launchctl stop org.macports.nrpe && launchctl start org.macports.nrpe",
                path => ["/bin"],
                schedule => "daily",
        }
    }
